...
## ストーリー「さいごの満月」
取り壊しが決まったネオン路地裏。姿を見せなくなった師匠を待ちながら、チビ（キミ）は路地の仲間たちと
最後のまねっこ勝負をしていく、ちょっと切ない全10ステージ。

- ステージごとに会話 → CPU戦 → 勝ち／負けの会話。負けたら「もう一回」かマップへ
- 相手が進むほど強くなる（まねの正確さ・ごろにゃーを使う頻度が上がる）。最終ステージは師匠
- クリア状況はブラウザに保存。10ステージ目をクリアするとエンディング（ネオンがひとつずつ消えていく）
- 会話は クリック / タップ / Space で進む、Esc か「スキップ」で飛ばせる
- 台本とステージ設定は `js/story/StoryData.js`
