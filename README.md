...
- ほかに **CPUと練習**（ノラ猫と1本勝負）と **ストーリー「さいごの満月」**（全10ステージ）あり
...
## ストーリー「さいごの満月」
満月の夜、ネオン路地裏は取り壊される。まねっこを教えてくれた師匠は、3日前から帰ってこない。
チビ（キミ）は「聞きたいことは、まねっこで勝ち取る」という路地の決まりに従って、一夜ずつ仲間に勝負を挑み、師匠の手がかりを集めていく。

- 各話は「起承転結」で閉じず、次の話の「起」（新しい謎や知らせ）で終わる構成
  （半分残された焼き魚 → 師匠の鈴 → 途切れた録音 → 汽笛 → ユキの家出 → ハチの隠しごと → 屋根の上の影 → 眠る背中 → 録音の続き → 満月）
- 取り壊しへの反応は猫それぞれ：居座る（タマ・ヨル）、もう引っ越したのに毎晩戻る（ネオ）、引っ越す予定（ミケ）、
  データに残そうとする（ピコ）、家出して抵抗（ユキ）、みんなの行き先を探す世話役（ハチ）、看板を灯して抵抗（ルナ）、見届ける（ハイじい）
- 毎話の最初に「満月まで あと○夜」の画面。ステージ1の前に師匠との出会いの回想
